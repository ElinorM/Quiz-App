...
describe('Quiz', () => {
  ...
  it('should display the ScorePage when the quiz is submitted', async () => {
    ...
    for (let i = 0; i < QUESTIONS.length - 1; i++) {
      await userEvent.click(screen.getByRole('radio', { name: QUESTIONS[i].options[2] }));
      await userEvent.click(screen.getByRole('button', { name: 'Next' }));
    }
    await userEvent.click(screen.getByRole('radio', { name: QUESTIONS[QUESTIONS.length - 1].options[2] }));
